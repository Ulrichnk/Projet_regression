...
test_standardized <- standardize_data(train_set)
...
```{r}
coef_df <- as.data.frame(summary(poisson_model_full)$coefficients)
...
dispersion <- sum(residuals(poisson_model_full, type = "pearson")^2) / poisson_model_full$df.residual
...
predicted_train_poisson_full <- predict(poisson_model_full, newdata = train_standardized, type = "response")
rmse_train_poisson_full <- sqrt(mean((train_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle poisson full :", rmse_reduced))


predicted_train_poisson_reduced <- predict(poisson_model_reduced, newdata = train_standardized, type = "response")
rmse_train_poisson_reduced <- sqrt(mean((train_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle poisson réduit :", rmse_reduced))


predicted_reduced <- predict(nb_model_full, newdata = train_standardized, type = "response")
rmse_reduced <- sqrt(mean((train_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle full nb :", rmse_reduced))

predicted_train_nb_reduced <- predict(nb_model_reduced, newdata = train_standardized, type = "response")
rmse_train_nb_reduced <- sqrt(mean((train_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle nb réduit :", rmse_reduced))
...
predicted_train_nb_reduced <- predict(nb_model_reduced, newdata = train_standardized, type = "response")
rmse_train_nb_reduced <- sqrt(mean((train_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle nb réduit :", rmse_train_nb_reduced))

predicted_test_nb_reduced <- predict(nb_model_reduced, newdata = test_standardized, type = "response")
rmse_test_nb_reduced <- sqrt(mean((test_standardized$Claim - predicted_reduced)^2))
print(paste("RMSE du modèle nb réduit :", rmse_test_nb_reduced))
```


```{r}
#RMSE_C
C_1 <- which(y %in% c(0, 1))  # Classe très fréquente
C_2 <- which(y == 2)          # Classe fréquente
C_3 <- which(y == 3)          # Classe rare
C_4 <- which(y > 3)           # Classe très rare

# Calcul des RMSE par classe
rmse_1_poisson <- Metrics::rmse(y[C_1], yhat_poisson[C_1])
rmse_2_poisson <- Metrics::rmse(y[C_2], yhat_poisson[C_2])
rmse_3_poisson <- Metrics::rmse(y[C_3], yhat_poisson[C_3])
rmse_4_poisson <- Metrics::rmse(y[C_4], yhat_poisson[C_4])

# Calcul du RMSE_C pour le modèle Poisson
RMSE_C_poisson <- (rmse_1_poisson + rmse_2_poisson + rmse_3_poisson + rmse_4_poisson) / 4
print(paste("RMSE_C du modèle Poisson :", RMSE_C_poisson))
```
