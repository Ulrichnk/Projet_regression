...
```{r}
data_scaled <- train_standardized
...
plot(hc, cex = 0.6, hang = -1, main = "Dendrogramme du clustering hiérarchique")

# Découpage du dendrogramme pour obtenir 3 clusters, par exemple
...
#install.packages(c("MASS", "AER", "ggplot2", "dplyr"))

# Charger les packages
library(MASS)    # Pour glm.nb
library(AER)     # Pour dispersiontest
...
```
evaluatoin de la surdipertion
Si la statistique de dispersion est significativement supérieure à 1, cela indique une surdispersion, et un modèle binomial négatif pourrait être plus approprié.

```{r}
# Test de surdispersion
disp_test <- dispersiontest(modele_poisson)
print(disp_test)

```
binomial
```{r}
# Prédictions avec le modèle choisi (ex. modèle binomial négatif)
train_standardized$predictions <- predict(modele_poisson, type = "response")

# Comparaison observée vs prédite
head(train_standardized[, c("Claim", "predictions")])


```
```{r}
modele_poisson%>% residuals(type='response') %>% summary()

```

```{r}
ggplot(train_standardized, aes(x = Claim, y = predictions)) +
  geom_point(alpha = 0.6) +
  geom_abline(intercept = 0, slope = 1, col = "red", linetype = "dashed") +
  labs(title = "Observé vs Prédit", x = "Nombre de sinistres observés", y = "Nombre de sinistres prédits") +
  theme_minimal()
